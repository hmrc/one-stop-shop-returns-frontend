...
class CountryOfConsumptionFromNiController @Inject()(
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = cc.authAndGetData {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(CountryOfConsumptionFromNiPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode, period: Period): Action[AnyContent] = cc.authAndGetData.async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(CountryOfConsumptionFromNiPage, value))
            _              <- cc.sessionRepository.set(updatedAnswers)
          } yield Redirect(CountryOfConsumptionFromNiPage.navigate(mode, updatedAnswers))
      )
  }
}
