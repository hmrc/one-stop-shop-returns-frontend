...
class ExternalController @Inject()(
  ...
  def onExternal(page: String, period: Option[Period] = None, lang: Option[String] = None, amountInPence: Option[Long] = None): Action[JsValue] =  cc.auth.async(parse.json) {
    implicit request =>
      withJsonBody[ExternalRequest] {
      externalRequest =>
      externalService.getExternalResponse(externalRequest, request.userId, page, period, lang, amountInPence) match {
