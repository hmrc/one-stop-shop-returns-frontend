...
class WantToUploadFileController @Inject()(
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = cc.authAndGetOptionalData(period) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.flatMap(_.get(WantToUploadFilePage)) match {
        case None => form
        ...
        value =>
          for {
            ...
            updatedAnswers <- Future.fromTry(request.userAnswers
              ...
              )).set(WantToUploadFilePage, value))
            _              <- cc.sessionRepository.set(updatedAnswers)
          } yield Redirect(WantToUploadFilePage.navigate(mode, updatedAnswers))
      )
  }
}
