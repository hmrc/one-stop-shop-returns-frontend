...
class VatOnSalesFromNiController @Inject()(
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = cc.authAndGetData {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(VatOnSalesFromNiPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode, period: Period): Action[AnyContent] = cc.authAndGetData.async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(VatOnSalesFromNiPage, value))
            _              <- cc.sessionRepository.set(updatedAnswers)
          } yield Redirect(VatOnSalesFromNiPage.navigate(mode, updatedAnswers))
      )
  }
}
