...
class CheckBouncedEmailFilterImpl()(implicit val executionContext: ExecutionContext)
  ...
  override protected def filter[A](request: RegistrationRequest[A]): Future[Option[Result]] = {
    ...
    if (request.registration.unusableStatus.getOrElse(false)) {
      Future(Some(Redirect(routes.InterceptUnusableEmailController.onPageLoad())))
    } else {
      None.toFuture
    }
  }
...
class CheckBouncedEmailFilterProvider @Inject()(implicit executionContext: ExecutionContext) {
