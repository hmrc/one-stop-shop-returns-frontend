  ...
  "must return correct view model when" - {
    ...
    "there is one due payment" in {
      ...
      result.sections mustBe Seq(PaymentsSection(
        Seq(
          s"""You owe <span class="govuk-body govuk-!-font-weight-bold">&pound;1,000</span> for ${period.displayText(messages(app))}. You must pay this by ${period.paymentDeadlineDisplay}."""
        ),
        Some("Due Payments")
      ))
    ...
    "there is one due payment with unknown status" in {
      ...
      result.sections mustBe Seq(PaymentsSection(
        Seq(
          s"""You may still have VAT to pay for ${period.displayText(messages(app))}. You must pay this by ${period.paymentDeadlineDisplay}."""
        ),
        Some("Due Payments")
      ))
      result.link mustBe defined
    ...
    "there is one overdue payment" in {
      ...
      result.sections mustBe Seq(PaymentsSection(
        Seq(
          s"""You owe <span class="govuk-body govuk-!-font-weight-bold">&pound;1,000</span> for ${period.displayText(messages(app))}, which was due by ${period.paymentDeadlineDisplay}."""
        ),
    ...
    "there is one overdue payment with unknown status" in {
      ...
      result.sections mustBe Seq(PaymentsSection(
        Seq(
          s"""You may still have VAT to pay for ${period.displayText(messages(app))}, which was due by ${period.paymentDeadlineDisplay}."""
        ),
        Some("Overdue Payments")
      ))
      result.link mustBe defined
    ...
    "there is one due payment, and two overdue payments, one with unknown status" in {
      ...
      result.sections mustBe Seq(
        PaymentsSection(
        Seq(
          s"""You owe <span class="govuk-body govuk-!-font-weight-bold">&pound;1,000</span> for ${period3.displayText(messages(app))}. You must pay this by ${period3.paymentDeadlineDisplay}.""",
        ),
          Some("Due Payments")),
        PaymentsSection(
          Seq(
            s"""You may still have VAT to pay for ${period1.displayText(messages(app))}, which was due by ${period1.paymentDeadlineDisplay}.""",
            s"""You owe <span class="govuk-body govuk-!-font-weight-bold">&pound;1,000</span> for ${period2.displayText(messages(app))}, which was due by ${period2.paymentDeadlineDisplay}."""
          ),
          Some("Overdue Payments")
        )
      )
