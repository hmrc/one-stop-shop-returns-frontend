...
class CountryOfConsumptionFromEuControllerSpec extends SpecBase with MockitoSugar {
  ...
  "CountryOfConsumptionFromEu Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(CountryOfConsumptionFromEuPage(index, index), country).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(country), NormalMode, period, index, index)(request, messages(application)).toString
      }
    }
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(CountryOfConsumptionFromEuPage(index, index), country).success.value
