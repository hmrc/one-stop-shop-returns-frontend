...
class VatOnSalesFromEuControllerSpec extends SpecBase with MockitoSugar {
  ...
  private val formProvider = new VatOnSalesFromEuFormProvider()
  private val form = formProvider()

  private val validAnswer = 0

  private lazy val vatOnSalesFromEuRoute = routes.VatOnSalesFromEuController.onPageLoad(NormalMode, period).url
  ...
  "VatOnSalesFromEu Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(VatOnSalesFromEuPage, validAnswer).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(VatOnSalesFromEuPage, validAnswer).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual VatOnSalesFromEuPage.navigate(NormalMode, expectedAnswers).url
