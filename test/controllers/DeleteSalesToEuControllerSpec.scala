...
import pages.{CountryOfConsumptionFromEuPage, CountryOfSaleFromEuPage, DeleteSalesToEuPage}
...
class DeleteSalesToEuControllerSpec extends SpecBase with MockitoSugar {
  ...
  "DeleteSalesToEu Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, deleteSalesToEuRoute)
        ...
        contentAsString(result) mustEqual view(form, NormalMode, period, index, index, countryFrom, countryTo)(request, messages(application)).toString
      }
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers = baseAnswers.set(DeleteSalesToEuPage(index, index), true).success.value
    ...
    "must delete a record and redirect to the next page when the user answers Yes" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, deleteSalesToEuRoute)
            .withFormUrlEncodedBody(("value", "true"))

        val result = route(application, request).value
        val expectedAnswers = baseAnswers.remove(SalesToEuQuery(index, index)).success.value
    ...
    "must not delete a record and redirect to the next page when the user answers No" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual DeleteSalesToEuPage(index, index).navigate(NormalMode, baseAnswers).url
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, NormalMode, period, index, index, countryFrom, countryTo)(request, messages(application)).toString
      }
    }
