  ...
  "Your Account Controller" - {
    ...
    "must return OK and the correct view with no saved answers" - {
      ...
      "when there are no returns due" in {
        ...
        when(returnStatusConnector.getCurrentReturns()(any())) thenReturn
          Future.successful(
            Right(OpenReturns(None, None, Seq.empty)))

        when(financialDataConnector.getCurrentPayments()(any())) thenReturn
          Future.successful(
      ...
      "when there is 1 nil return completed and payment is outstanding from a correction" in {
        ...
        when(financialDataConnector.getCurrentPayments()(any()))
