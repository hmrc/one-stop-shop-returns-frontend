...
class VatOnSalesFromNiControllerSpec extends SpecBase with MockitoSugar {
  ...
  "VatOnSalesFromNi Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(VatOnSalesFromNiPage(index, index), validAnswer).success.value
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(VatOnSalesFromNiPage(index, index), validAnswer).success.value
