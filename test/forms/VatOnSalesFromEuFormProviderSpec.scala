...
class VatOnSalesFromEuFormProviderSpec extends DecimalFieldBehaviours {
  ...
  private val form = new VatOnSalesFromEuFormProvider(mockVatRateService)(vatRate, netSales)
  ...
  "form" - {
    ...
    "when NonStandard is selected" - {
      ...
      "must not bind when a negative amount is supplied" in {
        ...
        result.errors must contain only FormError("amount", "vatOnSalesFromEu.amount.error.calculatedVatRateOutOfRange", Seq(0.01, maxCurrencyAmount))
      }
      ...
      "must not bind when a non-numeric amount is supplied" in {
        ...
        result.errors must contain only FormError("amount", "vatOnSalesFromEu.amount.error.nonNumeric")
      }
      ...
      "must not bind when a number with too many decimal places is supplied" in {
        ...
        result.errors must contain only FormError("amount", "vatOnSalesFromEu.amount.error.decimalFormat")
      }

      "must not bind when an amount is not supplied" in {

        val result = form.bind(Map("choice" -> NonStandard.toString))
        result.errors must contain only FormError("amount", "vatOnSalesFromEu.amount.error.required")
      }
    }
    ...
    "when no choice is selected" - {
      ...
      "must not bind" in {
        ...
        result.errors must contain only FormError("choice", "vatOnSalesFromEu.choice.error.required")
      }
    }
  }
}
